...
async function create_user(email, password, id, role) {
  const x = [{ message: " ", color: " " , flash: " " }];
  ...
  return new Promise((resolve, reject) => {
    ...
    const insertQuery = "INSERT INTO user (Email, Password, Id, Payload, Preset, Role) VALUES (?, ?, ?, ?, ?, ?)";
    const values = [email, password, id, payload, prelist, role];
...
async function set_message(message, email, Color) {
  const x = [{ message: message, color: Color , flash: " " }];
  const payload = JSON.stringify(x);
  return new Promise((resolve, reject) => {
...
async function edit_id(mail, id) {
  
  return new Promise((resolve, reject) => {
    connection.query("UPDATE user SET Id = ? WHERE Email = ?", [id, mail], function (err, data, _fields) {
      if (err) return reject(err);
...
async function get_message_email(mail) {
  return new Promise((resolve, reject) => {
    connection.query("SELECT * FROM user", function (err, data, _fields) {
      if (err) return reject(err);
      for (const item of data) {
...
async function edit_pre(num, message1, mail) {
...
async function put_flash(message1, mail) {
  let str = await get_message_email(mail);
  let obj = JSON.parse(str);
  obj[0].flash = message1;
  let payload = JSON.stringify(obj);
  return new Promise((resolve, reject) => {
    connection.query("UPDATE user SET Payload = ? WHERE Email = ?", [payload, mail], function (err, data, _fields) {
      if (err) return reject(err);
      resolve(true);
    });
  });
...
async function reset_flash(id) {
  let str = await get_message(id)
  let obj = JSON.parse(str)

  obj[0]["flash"] = " "
  let payload = JSON.stringify(obj);
  return new Promise((resolve, reject) => {
    connection.query("UPDATE user SET Payload = ? WHERE Id = ?", [payload, id], function (err, data, _fields) {
      if (err) return reject(err);
      resolve(true);
    });
  });
...
app.get('/hey/:id', async(req, res) => {
 ...
 let re = result[0].message + "*" + result[0].color + "*" + result[0].flash;
let srths = await reset_flash(id)
...
app.put('/editid/:mail/:Id', async(req, res) => {
  let mail = req.params.mail;
  let Id = req.params.Id;
  let mess = await edit_id(mail, Id)
 return res.send("Edited")
})
app.put('/flash/:mail/:word', async(req, res) => {
  let mail = req.params.mail;
  let word = req.params.word;
  let mess = await put_flash(word, mail)
...
app.put('/newacc/:email/:password/:id/:role', async (req, res) => {
    let email = req.params.email;
    let password = req.params.password;
    let id = req.params.id;
    let role = req.params.role;
    let c = await create_user(email, password, id, role)
    if(!c){
      return res.send("bad")}
    else
